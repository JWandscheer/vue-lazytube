<script>
import { getVimeoThumbnail, getYoutubeThumbnail, getYoutubeThumbnailSrcset } from "../../utils";
...
export default {
  ...
  computed: {
    ...
    thumbnailSrcsets() {
      if (this.type === 'youtube') {
        return getYoutubeThumbnailSrcset(this.videoID)
      } else {
        return getVimeoThumbnail(this.videoID, 'thumbnail_large').split(',')
      }
    },
  }
}
  ...
  <div class="vlt-preview" @click="$emit('click')">
    <template v-if="!onceLoaded">
      <template v-if="isVideoFound">
        ...
        <picture>
          <template v-for="[format, set] of Object.entries(thumbnailSrcsets)">
            <source v-if="format !== 'jpg'" :srcset="set" sizes="50vw" :type="'image/' + format">
          </template>
          <img loading="lazy" decoding="async" :srcset="thumbnailSrcsets['jpg']"
            :type="'image/jpg'" sizes="50vw"
            :alt="'Video - ' + videoTitle" width="100%">
        </picture>
        ...
        <button class="ly-button-wrapper" aria-label="Load and Play Video" v-show="!clicked">
          <slot name="button">
            ...
            <svg v-else height="100%" version="1.1" viewBox="0 0 68 48" width="100%" alt="Play Button">
